...
impl<'a, 'b> Painter<'a, 'b> {
    ...
    /// let cell = painter.get_cell(1.0, 0.0);
    /// assert_eq!(cell, Some((0, 7)));
    /// let cell = painter.get_cell(1.5, 1.0);
    /// assert_eq!(cell, Some((1, 3)));
    /// let cell = painter.get_cell(0.0, 0.0);
    /// assert_eq!(cell, None);
    /// let cell = painter.get_cell(2.0, 2.0);
    /// assert_eq!(cell, Some((3, 0)));
    /// let cell = painter.get_cell(1.0, 2.0);
    /// assert_eq!(cell, Some((0, 0)));
    /// ```
    pub fn get_cell(&self, x: f64, y: f64) -> Option<(usize, usize)> {
